...
__global__ void device_calculate(int *d_result, double d_xleft, double d_yupper, double d_step) {
	int i = blockIdx.x * BLOCKX + threadIdx.x;
	int j = blockIdx.y * BLOCKY + threadIdx.y;
	my_complex_t c,z,temp;
	int iter=0;
	c.real = (d_xleft + d_step*i);
	c.imag = (d_yupper - d_step*j);
	z = c;
...
int main(int argc,char **argv) {
	...
	cudaMemcpy(device_pixel, result, XSIZE * YSIZE * sizeof(int), cudaMemcpyDeviceToHost);
