							  ...
							  unsigned int const x)
{
	...
	// Exit condition: dwell is maxDwell or |z| >= 4. Remove metod call
	while (dwell < maxDwell && cabs(z) < 4)
	{
		// z = z² + initValue
		z = (z * z) + z0; //computeNextValue(z, z0); removed method init call out of loop, prevent jump by inline calculation
		dwell += dwellInc;
	}
...
void computeDwellBuffer(unsigned long long **buffer, double complex cmin, double complex cmax)
{
	...
	// Changed order of for-loops as the x values are continously from each y
	for (unsigned int y = 0; y < res; y++)
	{
		for (unsigned int x = 0; x < res; x++)
		{
			// Moved the valled methods into this for loop for less branching.
			double real = ((double)x / res) * creal(cmax - cmin) + creal(cmin);
			double imag = ((double)y / res) * cimag(cmax - cmin) + cimag(cmin);
	...
	clock_t end_func = clock();
	double time = (((double)(end_func - start_func) / CLOCKS_PER_SEC) * 1000.0);
	time = (int)time;
	time /= 1000;
	printf("computeDwellBuffer time: %f\n", time);
}
...
void mapDwellBuffer(bmpImage *image, unsigned long long **buffer)
{
	...
	for (unsigned int y = 0; y < res; y++)
	{
		for (unsigned int x = 0; x < res; x++)
		{
			...
			double complex z = x + y * I;
			unsigned long long index = buffer[y][x] + 1 - log(log(cabs(z) / log2));
...
exit_graceful:
	...
	double execute_time = ((double)end_time - (double)start_time) / (double)CLOCKS_PER_SEC;
