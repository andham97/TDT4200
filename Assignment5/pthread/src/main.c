...
pthread_cond_t completed_job;
bool running;
int number_threads_lock = 0;
...
void *worker(void *id)
{
	while (running)
	{
		...
		while (jobQueueHead == NULL)
		{
			...
			number_threads_lock++;
			pthread_cond_signal(&completed_job);
			pthread_cond_wait(&new_job, &queue_lock);
			number_threads_lock--;
		}
...
void initializeWorkers(pthread_t *thread_handle, unsigned int threads_number)
{
	pthread_mutex_init(&queue_lock, NULL);
	pthread_cond_init(&new_job, NULL);
	pthread_cond_init(&completed_job, NULL);
	for (unsigned int i = 0; i < threads_number; i++)
	{
		pthread_create(&thread_handle[i], NULL, worker, NULL);
	}
}
...
void *escape_thread(void **args)
{
	for (unsigned int x = 0; x < *((unsigned int *)args[1]); x++)
	{
		escapeTime(args[0], *((unsigned int *)args[2]) * *((unsigned int *)args[3]), x * *((unsigned int *)args[3]), *((unsigned int *)args[3]));
	}
	return NULL;
}
...
int main(int argc, char *argv[])
{
	...
	thread_handle = malloc(useThreads * sizeof(pthread_t));
	initializeWorkers(thread_handle, useThreads);
	...
	while (running)
	{
		if (number_threads_lock >= useThreads)
		{
			running = false;
			pthread_cond_broadcast(&new_job);
			pthread_mutex_unlock(&queue_lock);
			break;
		}
		pthread_cond_wait(&completed_job, &queue_lock);
	}
